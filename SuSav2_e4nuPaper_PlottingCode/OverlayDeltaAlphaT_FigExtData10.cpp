...
void OverlayDeltaAlphaT_FigExtData10() {
		...
		for (int WhichPlot = 0; WhichPlot < NPlots; WhichPlot ++) {
			...
			for (int WhichEnergy = 0; WhichEnergy < NEnergies; WhichEnergy ++) {
				...
				for (int WhichNucleus = 0; WhichNucleus < NNuclei; WhichNucleus ++) {
					...
					if (nucleus[WhichNucleus] == "56Fe") { MaxHeight = MaxHeight * 1.2; }
...
//		TPad* pad4GeV = new TPad("pad4GeV","pad4GeV",0.73,0.89,0.9,0.99,21); 
		...
		latexPmiss.DrawLatexNDC(0.1,0.5,"(e,e'p)_{1p0#pi} #delta#alpha_{T} [deg]");
		...
		PlotCanvas->cd();
		TPad* padTitleOne = new TPad("padTitleOne","padTitleOne",0.375,0.305,0.42,0.355,21); 
		...
		PlotCanvas->cd();
		TPad* padTitleTwo = new TPad("padTitleTwo","padTitleTwo",0.375,0.445,0.42,0.495,21); 
		...
		PlotCanvas->SaveAs("../../myPlots/pdf/"+xBCut[WhichxBCut]+"/"+version+ext+"DeltaAlphaT_FigExtData10"+WhatModelsAreIncluded+"_SuSav2.pdf");
