...
void Subtraction::prot1_pi1_rot_func(TVector3  V3prot,TVector3 V3pi, int q_pi,bool radstat, double *N_pi_p,double *N_nopi_p){
...
void Subtraction::prot1_pi2_rot_func(TVector3  V3prot,TVector3 V3pi[2], int q_pi[2],bool radstat[2], double *P_1p0pi,double P_1p1pi[2]){
    ...
    bool status_pi[2]={true};
  	 ...
  	 prot1_pi1_rot_func(V3prot,V3pi[h],q_pi[h],radstat[h],&N_pi_p,&N_nopi_p);
...
void Subtraction::prot1_pi3_rot_func(TVector3  V3prot,TVector3 V3pi[3], int q_pi[3],bool radstat[3],double *P_tot){
   ...
   //----------------------1p3pi->1p2pi->1p1pi->1p0pi
  	 prot1_pi2_rot_func(V3prot,V3_pion, q_pion,rad_stat,&P_1p0pion,P_1p1pion);
...
void Subtraction::prot2_pi1_rot_func(TVector3 V3_2prot_corr[2],TVector3 V3_2prot_uncorr[2],TVector3 V3_1pi, int q_pi,bool radstat,TLorentzVector V4_el, double Ecal_2p1pi_to2p0pi[2],double p_miss_perp_2p1pi_to2p0pi[2],double P_2p1pito2p0pi[2],double P_2p1pito1p1pi[2],double P_2p1pito1p0pi[2],double *P_tot){
   ...
   //---------------------------------- 2p 1pi ->2p 0pi   ----------------------------------------------
    ...
    prot2_rot_func(V3_2prot_corr,V3_2prot_uncorr, V4_el,Ecal_2p1pi_to2p0pi,p_miss_perp_2p1pi_to2p0pi,P_2pto1p ,&N_2p_det);
  	...
  	prot1_pi1_rot_func(V3_2prot_uncorr[z],V3_1pi, q_pi,radstat, &N_pidet,&N_piundet);
...
void Subtraction::prot2_pi2_rot_func(TVector3 V3_2prot_corr[2],TVector3 V3_2prot_uncorr[2],TVector3 V3_2pi[2], int q_pi[2],bool radstat[2],TLorentzVector V4_el, double Ecal_2p2pi[2],double p_miss_perp_2p2pi[2],double P_tot_2p[2]){
  ...
  for(int k=0;k<N_2pi;k++){
    N_pidet=N_piundet=0;
    prot1_pi1_rot_func(V3_2prot_uncorr[z],V3_2pi[k], q_pi[k],radstat[k],&N_pidet,&N_piundet);
   ...
   prot1_pi2_rot_func(V3_2prot_uncorr[z],V3_2pi,q_pi,radstat,&P_1p0pi,P_1p1pi);
   ...
   prot2_pi1_rot_func(V3_2prot_corr,V3_2prot_uncorr,V3_2pi[z], q_pi[z],radstat[z],V4_el,Ecal_2p2pi,p_miss_perp_2p2pi,P_2p1pito2p0pi, P_2p1pito1p1pi, P_2p1pito1p0pi,&Ptot);
...
void Subtraction::prot3_pi1_rot_func(TVector3 V3_3prot_corr[3],TVector3 V3_3prot_uncorr[3],TVector3 V3_pi, int q_pi,bool radstat,TLorentzVector V4_el, double Ecal_3p1pi[3],double p_miss_perp_3p1pi[3],double P_tot_3p[3]){
  ...
  //----------------------------------3p 1pi ->3p 0pi->1p0pi   ----------------------------------------------
   ...
   //---------------------------------- 3p 1pi ->1p 1pi   ----------------------------------------------
     ...
     N_pidet=N_piundet=0;
     prot1_pi1_rot_func(V3_3prot_uncorr[z],V3_pi, q_pi,radstat, &N_pidet,&N_piundet);
   ...
   prot2_pi1_rot_func(V3_2p_corr,V3_2p_uncorr,V3_pi, q_pi,radstat,V4_el,Ecal_2p1pi,p_miss_perp_2p1pi,P_2p1pito2p0pi, P_2p1pito1p1pi, P_2p1pito1p0pi,&Ptot);
...
void Subtraction::pi1_rot_func(TVector3 V3_pi, int q_pi,bool radstat, double *P_pi){
...
void Subtraction::pi2_rot_func(TVector3 V3_pi[2], int q_pi[2],bool radstat[2], double *P_0pi,double P_1pi[2]){
   ...
   double N_bothpi=0,N_nopi=0,N_1pi[N_pi]={0},P_pi1[N_pi]={0};
      ...
      pi1_rot_func(V3_pi[0],  q_pi[0], radstat[0], P_pi1);
      pi1_rot_func(V3_pi[1],  q_pi[1], radstat[1], P_pi1+1);
...
void Subtraction::pi3_rot_func(TVector3 V3_pi[3], int q_pi[3],bool radstat[3], double *P_0pi, double P_1pi[3],double P_320[3],double P_3210[][2]){
   ...
   //---------------------------3pi->1pi->0pi----------------------------------------------
      for(int h=0;h<N_pi;h++){
        pi1_rot_func( V3_pi[h],q_pi[h],radstat[h],&P_pi);
  	...
  	rad_stat[0]=radstat[1];rad_stat[1]=radstat[2];
        ...
        pi2_rot_func( V3_pion,q_pion,rad_stat,&P_0pion, P_1pion);
...
void Subtraction::pi4_rot_func(TVector3 V3_pi[4], int q_pi[4],bool radstat[4], double *P_0pi,double *P_410,double *P_420,double *P_4210,double *P_430,double *P_4310,double *P_4320,double *P_43210){
  ...
  if(!radstat[0] || !radstat[1]  || !radstat[2]|| !radstat[3]){
   ...
   //---------------------------4pi->1pi->0pi----------------------------------------------
  	...
  	rad_stat[0]=radstat[1];rad_stat[1]=radstat[2];rad_stat[2]=radstat[3];
        ...
        pi3_rot_func(V3_pion,q_pion,rad_stat,&P_0pion, P_1pion,P_320_pion,P_3210_pion);
  ...
  //---------------------------4pi->2pi->0pi----------------------------------------------
   ...
   pi2_rot_func( V3_2pi,q_2pi,radstat_2pi,&P_0pi, P_1pi);
