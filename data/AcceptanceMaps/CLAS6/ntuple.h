...
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("AcceptanceMaps_"+particle+".root");
      if (!f || !f->IsOpen()) {
         f = new TFile("AcceptanceMaps_"+particle+".root");
      }
...
void ntuple::Init(TTree *tree)
{
   ...
   fChain->SetBranchAddress("Acc", &Acc, &b_Acc);
   Notify();
}
