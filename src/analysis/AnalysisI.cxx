...
bool AnalysisI::Analyse(Event &event)
{
  ...
  if (kElectronFit && out_mom.Theta() * 180 / TMath::Pi() < GetElectronMinTheta(out_mom))
    return false;
  ...
  if (!utils::IsValidSector(out_mom.Phi(), kEBeam, UseAllSectors()))
    return false;
  if (!utils::IsValidSector(out_mom.Phi(), EnabledSectors()))
    return false;

  if (ApplyOutElectronCut())
  {
    if (out_mom.P() < conf::GetMinMomentumCut(conf::kPdgElectron, kEBeam))
      return false;
  }

  if (ApplyThetaSlice())
  {
    if (out_mom.Theta() * 180. / TMath::Pi() < GetEThetaSliceMin())
      return false;
    if (out_mom.Theta() * 180. / TMath::Pi() > GetEThetaSliceMax())
      return false;
  }

  if (ApplyPhiOpeningAngle())
  {
    if (!conf::ValidPhiOpeningAngle(out_mom.Phi()))
      return false;
  }

  if (ApplyGoodSectorPhiSlice())
  {
    if (!conf::GoodSectorPhiSlice(out_mom.Phi()))
      return false;
  }
...
void AnalysisI::ApplyMomentumCut(Event &event)
{
  ...
  for (auto it = unsmeared_part_map.begin(); it != unsmeared_part_map.end(); ++it)
  {
    ...
    for (unsigned int i = 0; i < unsmeared_part_map[it->first].size(); ++i)
    {
      ...
      if (unsmeared_part_map[it->first][i].P() <= conf::GetMinMomentumCut(it->first, GetConfiguredEBeam()))
        continue;

      // Apply photon cuts for MC and data
      if (it->first == conf::kPdgPhoton)
      {
        if (!conf::ApplyPhotRadCut(out_mom, unsmeared_part_map[it->first][i]))
          continue;
      }
...
void AnalysisI::CookEvent(Event &event)
{
  ...
  if (part_map.find(conf::kPdgPi0) != part_map.end())
    std::cout << "NOTICE: Pi0 present in generation file. They should be decayed" << std::endl;
  std::map<int, unsigned int> Topology = GetTopology();
...
void AnalysisI::PlotBkgInformation(Event event)
{
  ...
  {
    if (!kHistograms[tags[obs_id]][kid_totestbkg] || !kHistograms[tags[obs_id]][kid_signal] || !kHistograms[tags[obs_id]][kid_tottruebkg] || !kHistograms[tags[obs_id]][kid_2p0pitruebkg] || !kHistograms[tags[obs_id]][kid_1p1pitruebkg] || !kHistograms[tags[obs_id]][kid_2p1pitruebkg] || !kHistograms[tags[obs_id]][kid_1p2pitruebkg] || !kHistograms[tags[obs_id]][kid_2p0piestbkg] || !kHistograms[tags[obs_id]][kid_1p1piestbkg] || !kHistograms[tags[obs_id]][kid_2p1piestbkg] || !kHistograms[tags[obs_id]][kid_1p2piestbkg])
      return;
    ...
    if (event.IsBkg() == true)
    {
      ...
      {
        bool is_m_bkg = true;
        if ((AnalysisRecord[original_mult + kid_bkgcorr].first).size() != min_mult)
          is_m_bkg = false;
        ...
        std::vector<int> pdgs;
        if ((record_afiducials.first).size() == original_mult)
          pdgs = record_afiducials.first; // It comes directly from background event
        else if ((AnalysisRecord[original_mult + 1 + kid_bkgcorr].first).size() == original_mult)
          pdgs = AnalysisRecord[original_mult + 1 + kid_bkgcorr].first;
        ...
        for (unsigned int k = 0; k < pdgs.size(); ++k)
        {
          ...
          if (particle_pdg == conf::kPdgProton)
            ++nprotons;
          else if (particle_pdg == conf::kPdgPiP || particle_pdg == conf::kPdgPiM || particle_pdg == conf::kPdgPi0 || particle_pdg == conf::kPdgPhoton)
            ++npions;
        }
        ...
        if (original_mult == 2)
        {
          ...
          if (nprotons == 2 && npions == 0)
            kHistograms[tags[obs_id]][kid_2p0piestbkg]->Fill(event.GetObservable(tags[obs_id]), -event.GetTotalWeight());
          if (nprotons == 1 && npions == 1)
            kHistograms[tags[obs_id]][kid_1p1piestbkg]->Fill(event.GetObservable(tags[obs_id]), -event.GetTotalWeight());
        }
        else if (original_mult == 3)
        {
          // Store according to initial topology
          if (nprotons == 2 && npions == 1)
            kHistograms[tags[obs_id]][kid_2p1piestbkg]->Fill(event.GetObservable(tags[obs_id]), -event.GetTotalWeight());
          if (nprotons == 1 && npions == 2)
            kHistograms[tags[obs_id]][kid_1p2piestbkg]->Fill(event.GetObservable(tags[obs_id]), -event.GetTotalWeight());
        }
    ...
    else
    {
      ...
      else
      {
        ...
        if (kHistograms[tags[obs_id]][kid_tottruebkg + id])
          kHistograms[tags[obs_id]][kid_tottruebkg + id]->Fill(event.GetObservable(tags[obs_id]), event.GetTotalWeight());
        ...
        if ((record_amomcuts.first).size() == 2)
        {
          // Store according to initial topology
          if (nprotons == 2 && npions == 0)
            kHistograms[tags[obs_id]][kid_2p0pitruebkg]->Fill(event.GetObservable(tags[obs_id]), event.GetTotalWeight());
          if (nprotons == 1 && npions == 1)
            kHistograms[tags[obs_id]][kid_1p1pitruebkg]->Fill(event.GetObservable(tags[obs_id]), event.GetTotalWeight());
        }
        else if ((record_amomcuts.first).size() == 3)
        {
          // Store according to initial topology
          if (nprotons == 2 && npions == 1)
            kHistograms[tags[obs_id]][kid_2p1pitruebkg]->Fill(event.GetObservable(tags[obs_id]), event.GetTotalWeight());
          if (nprotons == 1 && npions == 2)
            kHistograms[tags[obs_id]][kid_1p2pitruebkg]->Fill(event.GetObservable(tags[obs_id]), event.GetTotalWeight());
        }
      }
    }
  }
...
void AnalysisI::Initialize(void)
{
  ...
  else
  {
    if (Ebeam == 1.161)
    {
      kElectronFit->SetParameters(17, 7);
    }
    else if (Ebeam == 2.261)
    {
      kElectronFit->SetParameters(16, 10.5);
    }
    else if (Ebeam == 4.461)
    {
      kElectronFit->SetParameters(13.5, 15);
    }
    else
    {
      kElectronFit->SetParameters(13.5, 15);
...
bool AnalysisI::ApplyFiducialCut(Event &event, bool apply_fiducial)
{
  ...
  TLorentzVector out_mom = event.GetOutLepton4Mom();
  if (!fiducial->FiducialCut(conf::kPdgElectron, GetConfiguredEBeam(), out_mom.Vect(), IsData(), apply_fiducial))
    return false;
  ...
  for (auto it = part_map.begin(); it != part_map.end(); ++it)
  {
    ...
    for (unsigned int i = 0; i < part_map[it->first].size(); ++i)
    {
      if (!fiducial->FiducialCut(it->first, GetConfiguredEBeam(), part_map[it->first][i].Vect(), IsData(), apply_fiducial))
        continue;
      ...
      if (fFidAngleShift != 0)
      {
        ...
        if (!fiducial->FiducialCut(it->first, GetConfiguredEBeam(), out_mom_part_shift.Vect(), IsData(), apply_fiducial))
          continue;
        out_mom_part_shift.SetPhi(part_map[it->first][i].Phi() - fFidAngleShift * TMath::Pi() / 180.);
        if (!fiducial->FiducialCut(it->first, GetConfiguredEBeam(), out_mom_part_shift.Vect(), IsData(), apply_fiducial))
          continue;
...
bool AnalysisI::ApplyFiducialCutExtra(Event &event)
{
  ...
  // apply theta cuts
  TLorentzVector out_mom = event.GetOutLepton4Mom();
  if (!fiducial->FiducialCutExtra(conf::kPdgElectron, GetConfiguredEBeam(), out_mom.Vect(), IsData()))
    return false;
  ...
  for (auto it = part_map.begin(); it != part_map.end(); ++it)
  {
    ...
    for (unsigned int i = 0; i < part_map[it->first].size(); ++i)
    {
      if (!fiducial->FiducialCutExtra(it->first, GetConfiguredEBeam(), part_map[it->first][i].Vect(), IsData()))
        continue;
      visible_part.push_back(part_map[it->first][i]);
      visible_part_uncorr.push_back(part_map_uncorr[it->first][i]);
    }
...
bool AnalysisI::StoreTree(Event event)
{
  ...
  // Angle between outgoing electron and had system
  double AngleElvsHad = utils::Angle(out_mom.Vect(), utils::TotHadron(hadron_map).Vect()) * TMath::RadToDeg();
  ...
  if (isFirstCall == true)
  {
    ...
    kAnalysisTree->Branch("AngleElvsHad", &AngleElvsHad, "AngleElvsHad/D");
