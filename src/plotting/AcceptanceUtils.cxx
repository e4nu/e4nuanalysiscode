                                        ...
                                        std::string analysis_id, bool store_root)
{
  ...
  for (unsigned int i = 0; i < mc_files.size(); ++i)
  {
    ...
    if (addbinning.size() > 0)
    {
      ...
      for (unsigned int k = 0; k < addbinning.size() - 1; k++)
      {
        ...
        if (k == 0)
          name += GetAlternativeObs(observable) + " < " + std::to_string(addbinning[k + 1]);
        else if (k == addbinning.size() - 2)
          name += GetAlternativeObs(observable) + " > " + std::to_string(addbinning[k]);
        else
          name += std::to_string(addbinning[k]) + " < " + GetAlternativeObs(observable) + " < " + std::to_string(addbinning[k]);
      ...
      for (int k = 0; k < NEntries; ++k)
      {
        ...
        if (addbinning.size() != 0)
        {
          for (unsigned int l = 0; l < addbinning.size() - 1; l++)
          {
            ...
            {
              if (j == initial_size_trees)
                hists_recoacc_slices[i][l]->Fill(content, w);
              else if (j == initial_size_trees + 1)
                hists_trueacc_slices[i][l]->Fill(content, w);
            }
          }
        }
      }
  ...
  if (ratios_slices.size() != 0)
  {
    for (unsigned l = 0; l < ratios_slices[0].size(); ++l)
    {
      std::string name = "MC Acceptance for ";
      if (l == 0)
        name += GetAlternativeObs(observable) + " < " + std::to_string(addbinning[l + 1]);
      else if (l == addbinning.size() - 2)
        name += GetAlternativeObs(observable) + " > " + std::to_string(addbinning[l]);
      else
        name += std::to_string(addbinning[l]) + " < " + GetAlternativeObs(observable) + " < " + std::to_string(addbinning[l]);
