...
void plotting::SetAnalysisBranch( TTree * tree ) {
  ...
  if(tree->GetBranch("TrueNCh"))  tree->SetBranchAddress("TrueNCh", &TrueNCh);
}
...
double plotting::GetObservable(const std::string observable)
{
  ...
  else if (observable == "BeamE") content = BeamE;
  else if (observable == "Efl") content = Efl;
  ...
  else if (observable == "pfl_T") content = pfl_T;
  ...
  else if (observable == "MissingTransMomentum") content = MissingTransMomentum;
  else if (observable == "CorrMissingEnergy") content = ComputeMissingEnergy( Efl, HadSystemMass );
  else if (observable == "CorrMissingEnergy1") content = ComputeMissingEnergy( Efl, HadSystemMass, 1 );
  else if (observable == "CorrMissingEnergy2") content = ComputeMissingEnergy( Efl, HadSystemMass, 2 );
  else if (observable == "CorrMissingEnergy3") content = ComputeMissingEnergy( Efl, HadSystemMass, 3 );
  ...
  else if (observable == "ElectronPT") content = ElectronPT;
  else if (observable == "PionPT") content = PionPT;
  else if (observable == "TrueNProtons") content = TrueNProtons;
  else if (observable == "TrueNNeutrons") content = TrueNNeutrons;
...
std::vector<double> plotting::GetBinning(std::string observable, double EBeam, std::string analysis_key)
{
  ...
  if (observable == "ECal") {
    if (EBeam > 0 && EBeam < 2 ) binning = plotting::GetECalBinning(10, 10, 0.6, EBeam + 0.15, EBeam);
    else if (EBeam > 2 && EBeam < 4) binning = plotting::GetECalBinning(10, 10, 0.6, EBeam + 0.15, EBeam);
    else if (EBeam > 4 && EBeam < 5) binning = plotting::GetECalBinning(10, 10, 1.8, EBeam + 0.15, EBeam);
  } else if (observable == "RecoEvPion") {
      if (EBeam > 0 && EBeam < 2 )
  ...
  {
    if (EBeam > 0 && EBeam < 2 ) binning = plotting::GetUniformBinning(10, 20, 50);
    else if (EBeam > 2 && EBeam < 4) binning = plotting::GetUniformBinning(10, 20, 50);
    else if (EBeam < 5 && EBeam > 4) binning = plotting::GetUniformBinning(10, 15, 40);
  }
  else if (observable == "pfl_phi")
  {
    if (EBeam > 0 && EBeam < 2 )
  ...
  {
    if (EBeam > 0 && EBeam < 2 ) binning = plotting::GetUniformBinning(10, 0.35, 0.9);
    else if (EBeam > 2 && EBeam < 4) binning = plotting::GetUniformBinning(10, 0.5, 1.9);
    else if (EBeam == 4.461) binning = plotting::GetUniformBinning(10, 1.1, 3.8);
  }
  else if (observable == "pfl_T")
  {
    if (EBeam > 0 && EBeam < 2 )
  ...
  {
    if (EBeam > 0 && EBeam < 2 ) binning = plotting::GetUniformBinning(10, 0.2001, 1.0999);
    else if (EBeam > 2 && EBeam < 4) binning = plotting::GetUniformBinning(10, 0.2001, 1.9999);
    else if (EBeam == 4.461) binning = plotting::GetUniformBinning(10, 0.2001, 2.9999);
  }
  else if (observable == "proton_theta")
  {
    if (EBeam > 0 && EBeam < 2 )
    binning = plotting::GetUniformBinning(10, 0.0001, 80.001);
    else if (EBeam > 2 && EBeam < 4)
    binning = plotting::GetUniformBinning(10, 0.0001, 80.001);
    else if (EBeam == 4.461)
    binning = plotting::GetUniformBinning(10, 0.0001, 80.001);
  ...
  {
    if (EBeam > 0 && EBeam < 2 ) binning = plotting::GetUniformBinning(10, 0.101, 0.61);
    else if (EBeam > 2 && EBeam < 4) binning = plotting::GetUniformBinning(10, 0.01, 1.61);
    else if (EBeam == 4.461) binning = plotting::GetUniformBinning(10, 0.01, 2.01);
  }
  else if (observable == "pim_theta")
  {
    if (EBeam > 0 && EBeam < 2 ) binning = plotting::GetUniformBinning(10, 5, 140);
    else if (EBeam > 2 && EBeam < 4) binning = plotting::GetUniformBinning(10, 5, 140);
    else if (EBeam == 4.461) binning = plotting::GetUniformBinning(10, 5, 140);
  }
  else if (observable == "pip_mom")
  {
    if (EBeam > 0 && EBeam < 2 )
    binning = plotting::GetUniformBinning(10, 0.1, 3);
    else if (EBeam > 2 && EBeam < 4)
    binning = plotting::GetUniformBinning(10, 0.1, 3);
    else if (EBeam == 4.461)
    binning = plotting::GetUniformBinning(10, 0.1, 3);
  ...
  {
    if (EBeam > 0 && EBeam < 2 ) binning = plotting::GetUniformBinning(15, 1, 1.49);
    else if (EBeam > 2 && EBeam < 4) binning = plotting::GetUniformBinning(15, 1, 1.9);
  ...
  {
    if (EBeam > 0 && EBeam < 2 ) binning = plotting::GetUniformBinning(13, 0.0001, 0.999);
    else if (EBeam > 2 && EBeam < 4) binning = plotting::GetUniformBinning(15, 0.0001, 0.999);
    else if (EBeam == 4.461) binning = plotting::GetUniformBinning(15, 0.0001, 1.4999);
  }
  else if (observable == "HadDeltaPTx")
  {
    if (EBeam > 0 && EBeam < 2 )
  ...
  {
    if (EBeam > 0 && EBeam < 2 ) binning = plotting::GetUniformBinning(10, 0, 180);
    else if (EBeam > 2 && EBeam < 4) binning = plotting::GetUniformBinning(10, 0, 180);
    else if (EBeam == 4.461) binning = plotting::GetUniformBinning(10, 0, 180);
  }
  else if (observable == "RecoEnergyTransfer")
  {
    if (EBeam > 0 && EBeam < 2 ) binning = plotting::GetUniformBinning(15, 0., 0.7);
    else if (EBeam > 2 && EBeam < 4) binning = plotting::GetUniformBinning(15, 0.2, 1.2);
  ...
  {
    if (EBeam > 0 && EBeam < 2 ) binning = plotting::GetECalBinning(15,10, -0.15, 0.47, 0.1);
    else if (EBeam > 2 && EBeam < 4) binning = plotting::GetECalBinning(10, 12, -0.15, 1.7, 0.2);
  ...
  {
    if (EBeam > 0 && EBeam < 2 ) binning = plotting::GetECalBinning(15, 10, -0.15, 1.5, 0.3);
    else if (EBeam > 2 && EBeam < 4) binning = plotting::GetECalBinning(10, 10, -0.15, 2.2, 0.35);
  ...
  if (analysis_key == "1p1pip")
  {
    ...
    {
      if (EBeam > 0 && EBeam < 2 ) binning = plotting::GetECalBinning(10, 10, 0.6, EBeam + 0.15, EBeam);
      else if (EBeam > 2 && EBeam < 4) binning = plotting::GetECalBinning(12, 10, 0.6, EBeam + 0.15, EBeam);
      else if (EBeam > 4 && EBeam < 5) binning = plotting::GetECalBinning(10, 10, 1.8, EBeam + 0.15, EBeam);
    }
    else if (observable == "proton_mom")
    {
    ...
    {
      if (EBeam > 0 && EBeam < 2 )
      binning = plotting::GetUniformBinning(10, 0, 80);
      else if (EBeam > 2 && EBeam < 4)
      binning = plotting::GetUniformBinning(10, 0, 80);
      else if (EBeam == 4.461)
      binning = plotting::GetUniformBinning(10, 0, 80);
    ...
    else if (observable == "HadDeltaPT" || observable == "DeltaPT")
    {
      ...
      else if (EBeam == 4.461) binning = plotting::GetUniformBinning(10, 0.0001, 1.4999);
    }
    else if (observable == "HadDeltaPhiT" || observable == "DeltaPhiT")
    {
      if (EBeam > 0 && EBeam < 2 ) binning = plotting::GetUniformBinning(15, 0, 80);
      else if (EBeam > 2 && EBeam < 4) binning = plotting::GetUniformBinning(15, 0, 80);
    ...
    {
      if (EBeam > 0 && EBeam < 2 ) binning = plotting::GetECalBinning(5,8, -0.15, 0.47, 0.1);
      else if (EBeam > 2 && EBeam < 4) binning = plotting::GetECalBinning(10, 15, -0.15, 1.7, 0.2);
    ...
    {
      if (EBeam > 0 && EBeam < 2 ) binning = plotting::GetECalBinning(5,8, -0.1, 1.5, 0.1);
      else if (EBeam > 2 && EBeam < 4) binning = plotting::GetECalBinning(5, 10, -0.15, 2, 0.15);
...
void plotting::StandardFormat(TH1D *prediction, std::string title, int color, int style, std::string observable, std::string units, bool is_log, double y_max, std::string y_axis_label )
{
  ...
  if (y_axis_label == "") prediction->GetYaxis()->SetTitle(GetAxisLabel(observable, 1, units).c_str());
  ...
  if( is_log ) {
    ...
    if( units == "nb") prediction->GetYaxis()->SetRangeUser(0.1, y_max);
  } else { prediction->GetYaxis()->SetRangeUser(0.1, y_max); }
  if( units == "nb") prediction->GetYaxis()->SetRangeUser(0.0001, y_max);
