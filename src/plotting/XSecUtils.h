...
namespace e4nu {
  namespace plotting {
    ...
    void Plot2DXSec(std::vector<std::string> MC_files_name, std::string data_file_name,
        ...
        std::map<std::string,std::vector<double>> cuts,
        std::string analysis_id = "default", std::string units = "mb", bool store_root = false, bool log_scale = false, bool scale_mott = false, double scaling = 1 ) ;
    ...
    void PlotXsecDataTotal( TH1D * data, std::string observable, std::string title, std::string data_name,
			    ...
			    std::string analysis_id, std::string units = "mb", bool store_root = false, bool log_scale = false ) ;
    ...
    void PlotPerSector( std::vector<TH1D*> mc_per_sector,std::vector<TH1D*> data_per_sector, std::string observable,
			...
			std::string input_MC_location, std::string input_data_location, std::string output_location,
			std::string output_file_name, std::map<string,double> systematic_map,
			std::string analysis_id = "default", std::string units = "mb", bool store_root = false, bool log_scale = false) ;

    void CreateCanvasWithPads(TPad *& pad, std::vector<TPad*>& topPad, std::vector<TPad*>& bottomPad, const std::string& canvasName);
    void PlotProjection( const std::vector<TH2D*>& mcHists, const std::vector<TH2D*>& data, const std::string& xobservable, const std::string& yobservable, TCanvas* canvas, std::string units, bool logScale, const std::string& axis, double y_cut_min, double y_cut_max );
    void PlotProjectionWithRatio( const std::vector<TH2D*>& mcHists, const std::vector<TH2D*>& breakdown, TH2D* data, TH2D* acceptance, TH2D* radcorr, const std::string& xobservable, const std::string& yobservable, TPad* topPad, TPad* bottomPad, std::string units, bool logScale, const std::string& axis, double y_cut_min, double y_cut_max, double scale );
    void PlotProjectionsStack( const std::vector<TH2D*>& mcHists, const std::vector<TH2D*>& breakdown, const TH2D* data, const TH2D* acceptance, const TH2D* radcorr, const std::string& xobservable, const std::string& yobservable, bool logScale, const std::string& axis, std::vector<double> y_cuts, const std::string& outputLocation, const std::string& outputName, std::string units, bool store_root, double scaling );
    void PlotSlicesGeneralized(const std::vector<TH2D*>& mcHists, const std::vector<TH2D*>& breakdown, TH2D* data, TH2D* acceptance, TH2D* radcorr,
        ...
        const std::string& outputName, std::string units, bool storeRoot, bool logScale, double scale );
