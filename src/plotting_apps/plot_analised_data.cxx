...
int plot_analised_data() {
    ...
    // Retrieve histograms
    TH1* hist1 = dynamic_cast<TH1*>(file1->Get("Data"));
    TH1* histdef = dynamic_cast<TH1*>(filedef->Get("Data"));
    ...
    for (int i = 1; i <= histdef->GetNbinsX(); ++i) { // Loop from bin 1 to nBins (excluding underflow and overflow bins)
        ...
        double average1 = (binContentdef + binContent1) * 0.5;

        double err2_1 = pow(binContentdef - average1, 2) + pow(binContent1 - average1, 2) * 0.5 - pow(binErrordef, 2) - pow(binError1, 2);
    ...
    herr1->Draw("HIST");
    herr1->GetYaxis()->SetTitle("#sigma_{RMS}/Def[%]");
