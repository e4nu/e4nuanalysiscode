...
for x in gst_file_names:
    ...
    script.write("ifdh cp -D "+gst_file_names[counter]+" $CONDOR_DIR_INPUT/genie.gst.root ;\n \n")
    script.write("git clone "+opts.GIT_LOCATION+" -b "+opts.BRANCH+" ;\n")
    script.write("cd e4nuanalysiscode ; source e4nu_gpvm_env.sh ; make ;\n")
    #write main command
    script.write("./process_radweights --input-gst-file $CONDOR_DIR_INPUT/genie.gst.root --output-gst-file $CONDOR_DIR_INPUT/rad_corr_"+str(counter)+".gst.root --true_EBeam "+str(opts.EnergyBeam)+" --target "+str(opts.TARGET)+" --rad-model "+opts.MODEL+" ; \n\n")
