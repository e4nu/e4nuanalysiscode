#ifndef treeProducer_data_h
#define treeProducer_data_h
...
//TString E = "1_161"; double Ebeam = 1.161;
TString E = "2_261"; double Ebeam = 2.261;
//TString E = "4_461"; double Ebeam = 4.461;

//TString Target = "3He";  
//TString Target = "4He";
TString Target = "12C";        
//TString Target = "56Fe";  
//TString Target = "208Pb";  

TString FSIModel = "Data";

//TString file_name = "./myFiles/"+E+"/"+FSIModel+"/"+xBCut+"/"+Target+"_"+E+"_"+FSIModel+"_Plots"+FSI;
TString file_name = Target+"_"+E+"_"+FSIModel+"_Plots_FSI";

//TString FullPath = "./mySamples/";
TString FullPath = "eresmaid_"+Target+"_"+E+"_"+FSIModel+"_FSI.root";


class treeProducer_data {
public :
   ...
   treeProducer_data(TTree *tree=0);
   virtual ~treeProducer_data();
...
#ifdef treeProducer_data_cxx
treeProducer_data::treeProducer_data(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {

	TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject(FullPath);
	if (!f || !f->IsOpen()) f = new TFile(FullPath);
...
treeProducer_data::~treeProducer_data()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t treeProducer_data::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t treeProducer_data::LoadTree(Long64_t entry)
{
...
void treeProducer_data::Init(TTree *tree)
{
...
Bool_t treeProducer_data::Notify()
{
   return kTRUE;
}

void treeProducer_data::Show(Long64_t entry)
{
   if (!fChain) return;
   fChain->Show(entry);
}
Int_t treeProducer_data::Cut(Long64_t entry)
{
   return 1;
}

#endif // #ifdef treeProducer_data_cxx
